...
public final class ForkedChannelEncoder
{
    ...
    {
        return new ForkedChannelEncoder( streamCharset, out, RERUN_TEST_AFTER_FAILURE );
    }
    ...
    public void stdOut( byte[] buf, int off, int len, boolean newLine )
    {
        ForkedProcessEvent event = newLine ? BOOTERCODE_STDOUT_NEW_LINE : BOOTERCODE_STDOUT;
        setOutErr( event.getOpcode(), buf, off, len, defaultCharset() );
    }
    ...
    public void stdOut( String msg, boolean newLine )
    {
        ...
        ForkedProcessEvent event = newLine ? BOOTERCODE_STDOUT_NEW_LINE : BOOTERCODE_STDOUT;
        setOutErr( event.getOpcode(), buf, 0, buf.length, UTF_8 );
    }

    public void stdErr( byte[] buf, int off, int len, boolean newLine )
    {
    ...
    private void setOutErr( String eventType, byte[] buf, int off, int len, Charset encoding )
    {
        StringBuilder event = stdOutErr( eventType, runMode.geRunName(), buf, off, len, encoding );
        encodeAndPrintEvent( event );
    }
